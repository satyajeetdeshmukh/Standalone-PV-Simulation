...
void takeinputs() {
  ...
  while (count < count_max_i) {
    Ib_read += ((double)(analogRead(pin_Ib) / 1023.0 * 5.0 - 2.4925 + 0.155 + 0.008) * 10 / count_max_i);
...
void non_mppt_func(){
  ...
  ki_term_nm += ki_nm * e_nm;
  kp_term_nm = kp_nm * e_nm;
  if (ki_term_nm > 1.1) {
    ki_term_nm = 1.1;
  }
  else if (ki_term_nm < -0.1) {
    ki_term_nm = -0.1;
  }
